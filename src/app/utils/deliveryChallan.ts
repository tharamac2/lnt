...
export const generateDeliveryChallanPDF = async (options: DeliveryChallanOptions) => {
  const { type, consignee, siteCode, date, remarks, items, filename } = options;
  const doc = new jsPDF();
  ...
  doc.setFontSize(8);
  doc.text('M. R. C. NO.', left + 2, subRowY + 5);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.text('M 25', left + 2, box2Top + 16);
  ...
  doc.text(instruction, (left + right) / 2, box2Bottom + 6, { align: 'center', maxWidth: width - 4 });

  let cursorY = box2Bottom + 10;
  if (remarks) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.text(`Remarks: ${remarks}`, left, cursorY, { maxWidth: width });
    cursorY += 5;
  }
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('GATE PASS COPY APPROVED BY', left + gatePassWidth / 2, y + 8, { align: 'center' });
  doc.text('TOTAL', left + gatePassWidth + 3, y + 8);
  y += 14;
  ...
  doc.text('(SIGNATURE OF RECEIVER)', left + 60, y + recHeight - 3);
  ...
  doc.text('1. CONSIGNEE', left + 30, y + 11);
  doc.text('3. GATE PASS (SECURITY - ACCOUNTS)', left + 90, y + 11);
  doc.text('2. CONSIGNEE - CONSIGNOR', left + 30, y + 15);
  doc.text("4. CONSIGNOR'S FILE", left + 90, y + 15);
